...
The minimum rate of pregnancy complications is `r min(preg_comp_stats)`%.

The maximum rate of pregnancy complications is `r max(preg_comp_stats)`%.

On average, the rate of pregnancy complications in India is `r mean(preg_comp_stats)`%.

The overall standard deviation of this data set is fairly low and the mean and median
are fairly close together therefore, we can assume that the data is well distributed.
Additionally, the lack of NA values leads us to believe that the data set was
collected in a consistent manner.

#### **Outliers**

A notable outlier is `r min(preg_comp_by_state_df$total_comp)` complications from 
`r unique(preg_comp_by_state_df %>% filter(total_comp==min(total_comp)) %>% pull(States))`. 
Other notable outliers are the four states with the highest complications, which
are notable due to the fact that it may be simply the case that these states are among the most populous states in India and therefore may have a larger amount of cases due to 
sheer numbers alone. 
...
- (Both df) How do states' total spending budget impact the number of menstrual related pregnancy complications?
- (Budegt df) For each year, which state contributes most into the budget?
- (Preg df) Which complication makes up the largest proportion for each state?

### States Spending vs. Number of Menstrual Related Pregnancy Complications

**How does  the amount of each state's budget for a particular year impact the number of menstrual related complications?**
...
Firstly, to begin our data wrangling, we had to manipulate our pregnancy complications data frame to filter out for the totals of each specific complication from each state. We then created a new data set by joining the pregnancy totals data frame with our menstrual budget data frame, joining by states and filtered by state, budget, and the total menstrual budget complications. <br>
...
population of India. There are only 11 out of the 23 states represented. This may be to the lack
of responses of budget data and pregnancy complication data from particular states. 
